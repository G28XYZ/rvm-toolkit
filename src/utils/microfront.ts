...
const DEFAULT_EXPORT_NAMES = ["default", "Microfront", "App", "Component"] as const;
 ...
 * ```
 */
export type MicrofrontComponentOptions<
  TProps = Record<string, never>,
  TModule extends Record<string, unknown> = MicrofrontComponentModule<TProps>,
> = {
...
const normalizeExportNames = (exportName?: MicrofrontComponentExportName): readonly string[] => {
  if (!exportName) return DEFAULT_EXPORT_NAMES;
  return typeof exportName === "string" ? [exportName] : exportName;
};

const isReactComponentLike = (value: unknown): boolean =>
  typeof value === "function" || Boolean(value && typeof value === "object" && "$$typeof" in value);
 ...
 * ```
 */
export const resolveMicrofrontComponent = <TProps = Record<string, never>>(
  module: Record<string, unknown> | null | undefined,
  exportName?: MicrofrontComponentExportName
): ComponentType<TProps> | undefined => {
  if (!module) return;

  for (const name of normalizeExportNames(exportName)) {
    const value = module[name];
    if (isReactComponentLike(value)) {
      return value as ComponentType<TProps>;
    }
  }
};
 ...
 * ```
 */
export const getMicrofrontComponent = async <
  TProps = Record<string, never>,
  TModule extends Record<string, unknown> = MicrofrontComponentModule<TProps>,
>(
  options: MicrofrontComponentOptions<TProps, TModule>
): Promise<ComponentType<TProps>> => {
  try {
    const module = await options.importer();
    return resolveMicrofrontComponent<TProps>(module, options.exportName) ?? options.fallback;
  } catch (error) {
 ...
 * ```
 */
export const lazyMicrofrontComponent = <
  TProps = Record<string, never>,
  TModule extends Record<string, unknown> = MicrofrontComponentModule<TProps>,
>(
  options: MicrofrontComponentOptions<TProps, TModule>
  ...
  lazy(async () => ({
    default: await getMicrofrontComponent(options),
  }));
